/*
 * Title: ClearCoreCommandProtocolEthernetTCP
 *
 * Objective:
 ...
 *	  connection through a switch or router), an ethernet crossover cable may be required.  
 * 2. [Optional] Debug messages are printed via the USB port. Use the Data Visualizer in 
 *	  Microchip Studio or another USB terminal program to view these messages.
 ...
 * 8. The ClearPath-SD motor(s) must be set to use the HLFB mode "ASG-Position
 *    w/Measured Torque" with a PWM carrier frequency of 482 Hz through the MSP
 *    software (select Advanced>>High Level Feedback [Mode]... then choose
 *    "ASG-Position w/Measured Torque" from the dropdown, make sure that 482 Hz
 *    is selected in the "PWM Carrier Frequency" dropdown, and hit the OK
 *    button).
 * 9. Set the Input Format in MSP for "Step + Direction".  
 * 10.Set the Input Resolution in MSP the same as your motor's Positioning
 ...
 * 11.Input and output devices can be wired to connectors IO0-IO5, DI6-DI8, and/or
 *    A9-A12. A table summarizing the acceptable connector modes for each
 ...
 * 		https://www.teknic.com/files/downloads/clearcore_user_manual.pdf
 * 
 * Copyright (c) 2026 Teknic Inc. This work is free to use, copy and distribute under the terms of
...
int main() {
	...
	//give the user time to open the USB port to view the IP address printout
	uint32_t timeout = 10000;
	uint32_t startTime = Milliseconds();
	while (!ConnectorUsb.PortIsOpen() && Milliseconds() - startTime < timeout) {
		continue;
	}
	...
	while (!EthernetMgr.PhyLinkActive()) {
		ConnectorUsb.SendLine("Physical Link (Ethernet cable) not yet detected...");
		Delay_ms(1000);
	}
	...
	if (usingDhcp) {
		...
		IpAddress ip = IpAddress(192, 168, 3, 174);
...
// 		EthernetMgr.NetmaskIp(netmask);
	...
	if (!client.Connect(serverIp, PORT_NUM)) {
		...
		client.Send("Setup successful\n");
		client.Send("Send 'h' to receive a list of valid commands\n");	
		}
	//==================================================================
	//========================/Ethernet TCP Setup========================
	//==================================================================
		...
		while (true) {
			...
			// Reestablish TCP connection if needed
			if(!client.Connected()){			
				uint32_t delay = 1000;
				if (!client.Connect(serverIp, PORT_NUM)) {
				...
				inputValid = true;
				i = 0;
				while(i<IN_BUFFER_LEN && client.Peek() != -1){
					input[i] = (char) client.Read();
					i++;
					Delay_ms(1);
				}
				...
				if (inputValid){
					...
					switch(input[0]){
						...
						case 'q':
							...
							switch(input[2]){
								...
								case 'P':
									...
									if (verboseFeedback){
										client.Send("Motor ");
										SendInt32asASCII(motorNum_in);
										client.Send(" is in position (steps) ");
									}
									SendInt32asASCII(queriedValue);
									client.Send("\n");
									break;
								...
								case 'V':
									...
									if (verboseFeedback){
										client.Send("Motor ");
										SendInt32asASCII(motorNum_in);
										client.Send(" is at velocity (steps/s) ");
									}
									SendInt32asASCII(queriedValue);
									client.Send("\n");
									break;
								...
								case 'S':
									...
									if (verboseFeedback){
										...
										SendInt32asASCII(statusReg.reg); // prints the status register in decimal
										client.Send("\n");
										if (statusReg.bit.AlertsPresent){
											SendInt32asASCII(alertReg.reg); // prints the alert register in decimal
											client.Send("\n");
										}
									}
									break;
						...
						case 'i':
							...
							if(verboseFeedback){
								client.Send("Connector ");
								SendInt32asASCII(connectorNum_in);
								client.Send(" value: ");
							}
							SendInt16asASCII(inputConnectorValue);
							client.Send("\n");
							break;
...
*/
 void SendFeedback(int32_t messageNumber){
	...
	if (verboseFeedback){
		client.Send(FeedbackMessages[messageNumber].message);
		client.Send("\n");
	} else {
		SendInt32asASCII(FeedbackMessages[messageNumber].number);
		client.Send("\n");
	}
 } // SendFeedback()
//------------------------------------------------------------------------------
...
void SendVerboseStatus(int32_t motorNumber) {
	...
	client.Send("Motor status register for motor M");
	SendInt32asASCII(motorNumber);
	client.Send(": ");
	SendInt32asASCII(statusReg.reg); // prints the status register in decimal
	client.Send("\n");
	
	client.Send("AtTargetPosition:	");
	SendInt32asASCII(statusReg.bit.AtTargetPosition);
	client.Send("\n");

	client.Send("StepsActive:     	");
	SendInt32asASCII(statusReg.bit.StepsActive);
	client.Send("\n");

	client.Send("AtTargetVelocity:	");
	SendInt32asASCII(statusReg.bit.AtTargetVelocity);
	client.Send("\n");
		
	client.Send("MoveDirection:   	");
	SendInt32asASCII(statusReg.bit.MoveDirection);
	client.Send("\n");

	client.Send("MotorInFault:    	");
	SendInt32asASCII(statusReg.bit.MotorInFault);
	client.Send("\n");

	client.Send("Enabled:         	");
	SendInt32asASCII(statusReg.bit.Enabled);
	client.Send("\n");

	client.Send("PositionalMove:  	");
	SendInt32asASCII(statusReg.bit.PositionalMove);
	client.Send("\n");

	client.Send("HLFB State:		");
	switch (statusReg.bit.HlfbState) {
		case 0:
			client.Send("HLFB_DEASSERTED");
			client.Send("\n");
			break;
		case 1:
			client.Send("HLFB_ASSERTED");
			client.Send("\n");
			break;
		case 2:
			client.Send("HLFB_HAS_MEASUREMENT");
			client.Send("\n");
			break;
		case 3:
			client.Send("HLFB_UNKNOWN");
			client.Send("\n");
			break;
		default:
	...
	client.Send("AlertsPresent:   	");
	SendInt32asASCII(statusReg.bit.AlertsPresent);
	client.Send("\n");
	...
	client.Send("Triggering:      	");
	SendInt32asASCII(statusReg.bit.Triggering);
	client.Send("\n");

	client.Send("InPositiveLimit: 	");
	SendInt32asASCII(statusReg.bit.InPositiveLimit);
	client.Send("\n");

	client.Send("InNegativeLimit: 	");
	SendInt32asASCII(statusReg.bit.InNegativeLimit);
	client.Send("\n");

	client.Send("InEStopSensor:   	");
	SendInt32asASCII(statusReg.bit.InEStopSensor);
	client.Send("\n");	
	...
	if (statusReg.bit.AlertsPresent){
		client.Send("Alert register:	");
		SendInt32asASCII(alertReg.reg); // prints the alert register in decimal
		client.Send("\n");

		client.Send("MotionCanceledInAlert:         ");
		SendInt32asASCII(alertReg.bit.MotionCanceledInAlert);
		client.Send("\n");

		client.Send("MotionCanceledPositiveLimit:   ");
		SendInt32asASCII(alertReg.bit.MotionCanceledPositiveLimit);
		client.Send("\n");

		client.Send("MotionCanceledNegativeLimit:   ");
		SendInt32asASCII(alertReg.bit.MotionCanceledNegativeLimit);
		client.Send("\n");

		client.Send("MotionCanceledSensorEStop:     ");
		SendInt32asASCII(alertReg.bit.MotionCanceledSensorEStop);
		client.Send("\n");

		client.Send("MotionCanceledMotorDisabled:   ");
		SendInt32asASCII(alertReg.bit.MotionCanceledMotorDisabled);
		client.Send("\n");

		client.Send("MotorFaulted:                  ");
		SendInt32asASCII(alertReg.bit.MotorFaulted);
		client.Send("\n");
 ...
 * SendInt32asASCII
 *
 *    Send a 32 bit value as a string that plays well with client.send
 *
 * Parameters:
 *    int32_t value - the value to send
 *
 * Returns: None
 */
void SendInt32asASCII(int32_t value) {
	char s[32]; //32 chars is more than enough
	snprintf(s, sizeof(s), "%ld", (long)value);
	client.Send(s);
}
//------------------------------------------------------------------------------
 ...
 * SendInt16asASCII
 *
 *    Send a 16 bit value as a string that plays well with client.send
 *
 * Parameters:
